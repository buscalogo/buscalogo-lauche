...
async function requestPageOrigins() {
  // Preferir pedir no popup (gesto do usuário). No SW o Chrome costuma negar.
    ...
    if (msg?.type === "BL_SET_PAGE_ALERT") {
      ...
      if (want && !(await hasPageOrigins())) {
        ...
        sendResponse({
          ...
          enabled: false,
          permission: false,
          message: "Permissão de páginas necessária. Ative o interruptor e confirme no diálogo do Chrome.",
        });
        return;
      }
