...
func Detect(cfg *config.Config) SystemInfo {
	info := SystemInfo{
		...
		AdminAvailable:  system.IsRoot() || system.HasCommand("setcap"),
	...
	if _, err := os.Stat("/etc/systemd/resolved.conf.d/99-buscalogo.conf"); err == nil {
		info.UsesBuscaLogoDNS = true
	}
	return info
}
...
// EnsureSystemIntegration — no Linux o drop-in/resolv já é persistente; no-op.
